...
ChunkFileResult_t CMapFile::LoadEntityCallback(CChunkFile *pFile, int nParam)
{
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		if (!strcmp("parallax_obb", pClassName))
		{
			...
			// Fixed to use Vmatrix. Don't use the SetIdentityMatrix or matrix3x4_t. Breaks map compiling
			...
			while (i < brush->numsides)
			{
				...
				//New code doesn't use MatrixSetColumn since we don't use matrix3x4_t anymore. Fixs assertion issue
				...
				// New function from Shiro. MatrixInversion was custom to the Parallax code
