...
#include "srcbox_shader_commands.h"
	...
	SHADER_DRAW
	{
		...
		if ( bDrawStandardPass )
		{
			...
			SHADOW_STATE
			{
				...
				//#define vsh_forgot_to_set_static_DOPIXELFOG 0
				//#define vsh_forgot_to_set_static_HARDWAREFOGBLEND 1
				//#define m_nDOPIXELFOG 0
				//#define m_nHARDWAREFOGBLEND 0

				//bool bDoPixelFog = 16 * m_nDOPIXELFOG + 32 * m_nHARDWAREFOGBLEND + 0;

				DECLARE_STATIC_VERTEX_SHADER(unlittwotexture_vs20);
				//SET_STATIC_VERTEX_SHADER_COMBO(DOPIXELFOG, bDoPixelFog);
				//SET_STATIC_VERTEX_SHADER_COMBO(HARDWAREFOGBLEND, bDoPixelFog);
			...
			DYNAMIC_STATE
			{
				...
				{
					#define GetWorldSpaceCameraDirection 0
					float vEyeDir[4];
					pShaderAPI->GetWorldSpaceCameraPosition(vEyeDir);
					//GetWorldSpaceCameraDirection(vEyeDir);
